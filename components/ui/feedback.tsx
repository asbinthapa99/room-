...
const Default = ({ label, onSubmit }: { label: string; onSubmit?: (feedback: { rating: number; message: string }) => void }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedEmoji !== null) {
      onSubmit?.({ rating: selectedEmoji, message });
  ...
  return (
    <div className="relative">
      ...
      <Material
        type="menu"
        className={clsx(
          'absolute bg-white w-[340px] duration-200 font-sans left-1/2 -translate-x-1/2 z-50',
          ...
          <div className="flex justify-between p-3 bg-gray-50 border-t border-gray-200">
            <span className="flex items-center justify-start cursor-pointer gap-1">
              <Button
                ...
                size="small"
                className={clsx(
                  'group hover:!bg-blue-300 !w-8 !h-8 p-0 duration-200 rounded-lg inline-flex items-center justify-center',
                  selectedEmoji === 0
              ...
              <Button
                ...
                size="small"
                className={clsx(
                  'hover:!bg-blue-300 hover:fill-blue-900 !w-8 !h-8 p-0 duration-200 rounded-lg inline-flex items-center justify-center',
                  selectedEmoji === 1
                    ? '!bg-blue-300 fill-blue-900'
                    : 'bg-transparent fill-gray-900'
                )}
            ...
            <Button type="submit" size="small" disabled={selectedEmoji === null}>
              Send
            </Button>
          </div>
        </form>
...
const Inline = ({ label, onSubmit }: { label: string; onSubmit?: (feedback: { rating: number; message: string }) => void }) => {
  ...
  return (
    <div
      className={clsx(
        'bg-white shadow-md border border-gray-200 duration-200 overflow-hidden w-fit flex flex-col justify-start rounded-full',
        expanded ? 'max-h-[280px] rounded-xl' : 'max-h-12'
      )}
      ref={ref}
    >
      <div
      ...
      >
        <p className="text-gray-900 text-sm whitespace-nowrap">{label}</p>
      ...
      {showContent && (
        <form onSubmit={handleSubmit} className="w-full transition-opacity duration-300 opacity-100">
...
export const Feedback = ({ type = 'default', label, onSubmit }: FeedbackProps) => {
  return type === 'inline' ? (
    <Inline label={label} onSubmit={onSubmit} />
  ) : (
    <Default label={label} onSubmit={onSubmit} />
  );
};
