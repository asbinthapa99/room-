...
export async function POST(req: Request) {
  ...
  if (!WEBHOOK_SECRET) return new NextResponse("Missing secret", { status: 500 });
  ...
  if (evt.type === "user.created" || evt.type === "user.updated") {
    ...
    await db.user.upsert({
      where: { clerkId: id },
      create: { clerkId: id, email: email ?? "", name, avatar: image_url ?? null },
      update: { email: email ?? "", name, avatar: image_url ?? null },
    });
  }
